## MIPS Assembly code for NS-Shaft Game
...
max_level:	.word 20
platform_y_speed_update_interval:	.word 5
gravity:	.word 2

# Player Configuration
player_id:	.word -1
player_size:	.word 20 30
player_locs:	.word 0 0
player_speed:	.word 0 0
player_move_on_platform_speed:     .word 8
player_falling_x_speed:		.word 4
...
# Platform System
platforms:      .word 0:40
platform_num:   .word 10
platform_size:  .word 100 15
initial_platform_number:	.word 7
initial_platform_data:	.word
			      ...
			      6 50  450 5
			      7 0   510 0
current_platform_address:	.word 0
platform_height_distance:	.word 60
platform_y_speed:		.word -1
initial_platform_y_speed:	.word -1
spring_platform_upward_speed:	.word -6
platform_rotation_speed:	.word 0
base_rotation_speed:		.word 1

# Timing System
time_landing_on_unstable_platform:	.word 0
time_unstable_platform_hold:	.word  500
time_player_getting_hurt:	.word 0
time_player_recover_from_hurt:	.word 3000
time_sleep:	.word 100

# Input System
input_key:	.word 0
move_key:	.word 0
buffered_move_key: .word 0
initial_move_iteration: .word 10
move_iteration: .word 0


.text
main:
	# Create the Game Object = load immediate 200 to $v0 = system call code
	li $v0, 200
	syscall
...
game_end_status:
	# $v0 holds the game status of current level, $v0 = 1: win and $v0 = 2: lose
	li $t1, 1
	li $t2, 2
	bne $v0, $t1, lose
...
lose:
	...
	# 2. call syscall 203 to display game lose text. The lose_text id is $a0 = -3 and location ($a1 = x_loc = 80, $a2 = y_loc = 280);
	li $a0, -3
	li $a1, 80
	li $a2, 280
	la $a3, game_lose
...
#    If type = 5, set "platform_rotation_speed" = "base_rotation_speed"
    ...
    # If type = 0
    beq $a0, 0, uplv_decrease  # t1 = count = 0
    beq $t0, $t1, plnp_normal_type
    # If type = 1
    addi $t1, $t1, 1           # t1 = count = 1
    beq $t0, $t1, plnp_flip_type
    # If type = 1
    addi $t1, $t1, 1           # t1 = count = 2
    beq $t0, $t1, plnp_spring_type
    # If type = 1
    addi $t1, $t1, 1           # t1 = count = 3
    beq $t0, $t1, plnp_spike_type
    # If type = 1
    addi $t1, $t1, 1           # t1 = count = 4
